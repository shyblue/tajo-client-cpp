...
TajoParser::TajoParser()
    : state(Start), bulkSize(0)
{
}

std::pair<size_t, TajoParser::ParseResult> TajoParser::parse(const char *ptr, size_t size)
{
    if( !arrayStack.empty() )
        return TajoParser::parseArray(ptr, size);
    else
        return TajoParser::parseChunk(ptr, size);
}

std::pair<size_t, TajoParser::ParseResult> TajoParser::parseArray(const char *ptr, size_t size)
{
    assert( !arrayStack.empty() );
    assert( !valueStack.empty() );

    long int arraySize = arrayStack.top();
    std::vector<TajoValue> arrayValue = valueStack.top().toArray();
    ...
    {
        std::pair<size_t, TajoParser::ParseResult>  pair = parseArray(ptr, size);
    ...
    for(; x < arraySize; ++x)
    {
        std::pair<size_t, TajoParser::ParseResult>  pair = parse(ptr + i, size - i);
...
std::pair<size_t, TajoParser::ParseResult> TajoParser::parseChunk(const char *ptr, size_t size)
{
    size_t i = 0;
        ...
        switch(state)
        {
            ...
            case ErrorLF:
                if( c == '\n')
                {
                    ...
                    valueStack.push(TajoValue(buf, tag));
                    return std::make_pair(i + 1, Completed);
                }
                else
                {
...
TajoValue TajoParser::result()
{
    assert( valueStack.empty() == false );

    if( valueStack.empty() == false )
    {
        TajoValue value = valueStack.top();
        valueStack.pop();
